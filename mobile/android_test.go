...
public class AndroidTest extends InstrumentationTestCase {
	...
	public void testAccountManagement() {
		...
		KeyStore ks = new KeyStore(getInstrumentation().getContext().getFilesDir() + "/keystore", CommerciumX.LightScryptN, CommerciumX.LightScryptP);
	...
	public void testInprocNode() {
		...
		try {
			...
			Node node = new Node(getInstrumentation().getContext().getFilesDir() + "/.CommerciumX", new NodeConfig());
			node.start();
	...
	// signatures too. Regression test for go-CommerciumX issue #14599.
...
func TestAndroid(t *testing.T) {
	...
	workspace, err := ioutil.TempDir("", "cmmx-android-")
	if err != nil {
	...
	// Generate the mobile bindings for CommerciumX and add the tester class
	gobind := exec.Command("gomobile", "bind", "-javapkg", "org.ethereum", "github.com/CommerciumBlockchain/go-CommerciumX/mobile")
	if output, err := gobind.CombinedOutput(); err != nil {
		t.Logf("%s", output)
		t.Fatalf("failed to run gomobile bind: %v", err)
	}
	build.CopyFile(filepath.Join("libs", "cmmx.aar"), "cmmx.aar", os.ModePerm)
