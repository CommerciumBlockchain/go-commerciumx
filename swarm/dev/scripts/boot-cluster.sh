...
#
# The cluster contains a bootnode, a cmmx node and multiple swarm nodes, with
...
# static cmmx configuration
CommerciumX_IP="192.168.33.3"
CommerciumX_RPC_PORT="7331"
CommerciumX_RPC_URL="http://${CommerciumX_IP}:${CommerciumX_RPC_PORT}"
...
# start_bootnode starts a bootnode which is used to bootstrap the cmmx and
# swarm nodes
start_bootnode() {
...
# start_CommerciumX_node starts a cmmx node with --datadir pointing at <base-dir>/cmmx
# and a single, unlocked account with password "cmmx"
start_CommerciumX_node() {
  local dir="${base_dir}/cmmx"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get cmmx account address"
  fi
  ...
  local args=(
    ...
    --rpc
    --rpcaddr   "${CommerciumX_IP}"
    --rpcport   "${CommerciumX_RPC_PORT}"
    --verbosity "6"
  )

  start_node "cmmx" "${CommerciumX_IP}" "$(which cmmx)" ${args[@]}
}
...
create_account() {
  ...
  cmmx --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
