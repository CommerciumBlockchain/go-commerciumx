...
// newCppEthereumGenesisSpec converts a go-CommerciumX genesis block into a Parity specific
// chain specification format.
func newCppEthereumGenesisSpec(network string, genesis *core.Genesis) (*cppEthereumGenesisSpec, error) {
	// Only ethash is currently supported between go-CommerciumX and cpp-ethereum
	...
	//spec.Params.BlockReward = (*hexutil.Big)(ethash.baseReward)
...
// newParityChainSpec converts a go-CommerciumX genesis block into a Parity specific
// chain specification format.
func newParityChainSpec(network string, genesis *core.Genesis, bootnodes []string) (*parityChainSpec, error) {
	// Only ethash is currently supported between go-CommerciumX and Parity
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
	// Reconstruct the chain spec in Parity's format
	...
	//spec.Engine.Ethash.Params.BlockReward = (*hexutil.Big)(ethash.baseReward)
	...
	//spec.Engine.Ethash.Params.EIP649Reward = (*hexutil.Big)(ethash.baseReward)
	...
	spec.Genesis.Seal.CommerciumX.Nonce = (hexutil.Bytes)(make([]byte, 8))
	binary.LittleEndian.PutUint64(spec.Genesis.Seal.CommerciumX.Nonce[:], genesis.Nonce)

	spec.Genesis.Seal.CommerciumX.MixHash = (hexutil.Bytes)(genesis.Mixhash[:])
	spec.Genesis.Difficulty = (*hexutil.Big)(genesis.Difficulty)
	spec.Genesis.Author = genesis.Coinbase
	spec.Genesis.Timestamp = (hexutil.Uint64)(genesis.Timestamp)
	spec.Genesis.ParentHash = genesis.ParentHash
...
// Python CommerciumX implementation.
...
// newPyEthereumGenesisSpec converts a go-CommerciumX genesis block into a Parity specific
// chain specification format.
func newPyEthereumGenesisSpec(network string, genesis *core.Genesis) (*pyEthereumGenesisSpec, error) {
	// Only ethash is currently supported between go-CommerciumX and pyethereum
	if genesis.Config.Ethash == nil {
		return nil, errors.New("unsupported consensus engine")
	}
