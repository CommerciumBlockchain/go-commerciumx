...
// Package les implements the Light CommerciumX Subprotocol.
package les
...
import (
	...
	"github.com/CommerciumBlockchain/go-CommerciumX/light"
	...
	"github.com/CommerciumBlockchain/go-CommerciumX/trie"
)
...
// NewProtocolManager returns a new ethereum sub protocol manager. The CommerciumX sub protocol manages peers capable
...
func (pm *ProtocolManager) Stop() {
	...
	log.Info("Stopping light CommerciumX protocol")
	...
	log.Info("Light CommerciumX protocol stopped")
}
...
func (pm *ProtocolManager) handle(p *peer) error {
	...
	if err := pm.peers.Register(p); err != nil {
		p.Log().Error("Light CommerciumX peer registration failed", "err", err)
		return err
	}
	defer func() {
	...
	for {
		if err := pm.handleMsg(p); err != nil {
			p.Log().Debug("Light CommerciumX message handling failed", "err", err)
			return err
		}
	}
}
...
func (pm *ProtocolManager) handleMsg(p *peer) error {
	...
	p.Log().Trace("Light CommerciumX message arrived", "code", msg.Code, "bytes", msg.Size)
...
// NodeInfo represents a short summary of the CommerciumX sub-protocol metadata
// known about the host peer.
type NodeInfo struct {
	Network    uint64              `json:"network"`    // CommerciumX network ID (1=Frontier, 2=Morden, Ropsten=3, Rinkeby=4)
